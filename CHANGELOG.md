...
- **`SEED-REBASE-01` (resolved):** regenerated the shipped seed dump `data/seed/ufc_corpus_v30.dump` on the promoted substrate (postgres:18; sha `72d13c23…`, 10.61 MB). Updated `data/seed/PROVENANCE.md`, the `.sha256` sidecar, and the `tests/integration/test_db_seed.py` golden row-counts (e.g. `round_stats` 68 960 → 69 684, `fights` 16 902 → 17 011); verified the round-trip restore into a disposable postgres:18 container matches the goldens exactly. A fresh `ufc db seed` now reproduces the promoted corpus.
