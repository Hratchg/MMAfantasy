...
**Model — AUDIT-01 re-baseline of `xgb_v2` (`RETRAIN-V31-01`, 2026-07-06):**
- Promoted the corrected-corpus `xgb_v2` candidate over the frozen artifact. `models/xgb_v2.joblib` was **deliberately re-baselined** — the AUDIT-01 byte-identity chain was intentionally broken and re-anchored: sha256 `6e7641…ba099` → `0b0b40…fecd`. Same locked `best_params` and 72-feature space, trained on a corrected substrate — the deduplicated `ufcstats` corpus (removing the 1.95× cross-source inflation), corrected implied-probability odds, BFO odds for 90/96 new fights, and Sherdog-seeded debutant Elo (1673 seeds). Note: the promoted model's probability calibration is `CalibratedClassifierCV(method="isotonic")`, where the prior frozen v2 used `method="sigmoid"` — a real characteristic change (the corrected-substrate refit selected isotonic), reflected in the Phase-26 `CALIB_00_CURRENT_STATE.json` audit and `test_calibration.py`. Gate-verified **PROMOTABLE**: hard gate PASS (Brier 0.20843 ≤ 0.2202, accuracy 0.6629 ≥ 0.6391), parity-or-better vs the dedup-refit baseline on all three slices, cross-cutoff leakage probe = 0. Prior v2 metrics (Brier 0.22062) were measured on the inflated corpus and are not a fair bar.
- Re-anchored every **live-artifact** SHA reference: `.planning/AUDIT-01-BASELINE-SHA.txt`, `models/xgb_v2-contract.json`, `predict.EXPECTED_XGB_V2_SHA`, `oof.EXPECTED_XGB_V2_SHA256`, `spike_noise_floor_v22.EXPECTED_XGB_V2_BRIER` (→ 0.20843), and the default-collected AUDIT-01 regression tests. Historical phase snapshots, frozen partner-contract snapshots, and sibling-model provenance intentionally retain the old SHA.
- **`META-REBASE-01` (resolved — retrained, not relabeled):** the `meta_v2` stacker was **retrained on the promoted base** — OOF regenerated via the new `xgb_v2` over the dedup corpus, meta refit across 5 seeds, gate **PASS** (`ship_outcome=PASS`, `stepwise_clears=True`), then promoted to `models/meta/meta_v2.joblib` (sha `77076d3b…` → `e04454…2502a8`). `base_model_sha256`, `meta_oof_parquet_sha256`, and `meta_input_distribution_hash` now genuinely reflect the new base (superseding the earlier interim relabel `2e683b9`); the Phase-34 `v2_3_slice_metrics` remeasure was re-run on the promoted stack. Re-anchored the `meta_v2` sha across the 5 `test_gate_verify_*` CLI tests, the sibling-script guards, and the partner contract. `meta_v2.joblib` is AUDIT-01 protected — promoted with `AUDIT01_OVERRIDE=1`. Meta remains **disabled** (`META_DISABLED_NO_LIFT=True`); enabling it is tracked separately as `META-ENABLE-01`.
- **`SEED-REBASE-01` (resolved):** regenerated the shipped seed dump `data/seed/ufc_corpus_v30.dump` on the promoted substrate (sha `fcd7a55c…`, 10.61 MB). Updated `data/seed/PROVENANCE.md`, the `.sha256` sidecar, and the `tests/integration/test_db_seed.py` golden row-counts (e.g. `round_stats` 68 960 → 69 684, `fights` 16 902 → 17 011). The dev DB is postgres:18 but CI seeds via `pg_restore 16`, which cannot read a pg_dump-18 archive (v1.16), so the shipped dump is produced in **PG16 custom format** (PG18 corpus bridged through a postgres:16 container); verified via a `pg_restore 16` round-trip into a fresh postgres:16 matching the goldens. A fresh `ufc db seed` now reproduces the promoted corpus.
