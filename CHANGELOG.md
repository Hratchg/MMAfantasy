...
- **Deferred — `SEED-REBASE-01`:** the shipped seed dump `data/seed/ufc_corpus_v30.dump` was **not** regenerated here (the DB moved to postgres:18 and a regen changes the seed row counts + `test_db_seed.py` goldens). Tracked separately; a fresh restore of the shipped dump still reproduces the *pre*-promotion corpus.
