...
- **Interim stop-gap — `META-REBASE-01`:** the frozen `meta_v2` stacker's `base_model_sha256` was relabeled to the new base so `ModelPredictor` construction (the `predictor.py` runtime base-binding guard) succeeds. The stacker was **not** retrained — its coefficients still reflect the old base's OOF. Safe today only because meta contributes nothing at inference (`META_DISABLED_NO_LIFT=True`); it **must be retrained on the new base before meta is ever enabled**. See `KNOWN_ISSUES.md → META-REBASE-01`.
