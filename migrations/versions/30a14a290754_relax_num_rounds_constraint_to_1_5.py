...
def upgrade() -> None:
    ...
    op.create_check_constraint("valid_num_rounds", "fights", sa.text("num_rounds BETWEEN 1 AND 5"))


def downgrade() -> None:
    """Downgrade schema."""
    op.drop_constraint("valid_num_rounds", "fights", type_="check")
    op.create_check_constraint("valid_num_rounds", "fights", sa.text("num_rounds IN (3, 5)"))
