#!/usr/bin/env python
"""Phase 29 / CAMP-V23-00 — Sherdog Association field coverage RE-AUDIT.

VERBATIM fork of ``scripts/audit_camp_v22.py`` per Phase 29 CONTEXT.md D-01
("Fork the Phase 20 audit script verbatim"). Three allowed edits only:
  1. This docstring header (Phase 20 → Phase 29, CAMP-V22-00 → CAMP-V23-00)
  2. ``AUDIT_VERSION = "v23.00"`` (was "v22.00")
  3. ``OUTPUT_PATH_DEFAULT`` points to the v23 phase directory

Everything else — the 6 D-06 thresholds, the 200-fighter sample, RANDOM_STATE=42,
the scope_recommendation ladder, the JSON schema, every helper function —
stays byte-identical to v22 per CONTEXT.md D-18 carry-forward ("no
post-measurement renegotiation"). The diff between v22 and v23 (excluding
those 3 allowed-edit anchors) is EMPTY by design.

References:
  - ROADMAP.md §Phase 29 (CAMP re-audit + EVAL infrastructure milestone)
  - REQUIREMENTS.md §CAMP-V23-00 (re-emit the audit artifact) +
    §CAMP-V23-01 (operator-binding scope decision in plan SUMMARY)
  - .planning/phases/29-camp-re-audit-eval-set-infrastructure/29-CONTEXT.md
    (D-01 fork, D-02 sample reuse, D-08 SHA chain, D-18 no renegotiation)

Locked decisions (carry-forward from Phase 20 CAMP-V22-00, unchanged in v23):
...
Anti-features (BANNED):
  ...
  - AF-3: model file mutation. ``models/xgb_v2.joblib`` is byte-identical
    (read-only); AUDIT-01 chain verifies SHA at phase end (D-08 MID + END).
...
xgb_v2.joblib`` is read-only; AUDIT-01 chain checkpoint runs in Plan 29-01
Task 2 via ``shasum -a 256 models/xgb_v2.joblib`` (MID + post-run).

Usage:
    uv run python scripts/audit_camp_v23.py \\
        --output-path .planning/phases/29-camp-re-audit-eval-set-infrastructure/CAMP_V23_AUDIT.json

    # Cache-only mode (no fresh fetches; for re-running off pre-warmed cache):
    uv run python scripts/audit_camp_v23.py --dry-run
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Phase 29 / CAMP-V23-00 — Sherdog Association field RE-AUDIT "
            "(200-fighter stratified sample; v23 fork of v22 per D-01)."
        ),
    )
    parser.add_argument(
        "--output-path",
        default=str(OUTPUT_PATH_DEFAULT),
        help="Output path for CAMP_V23_AUDIT.json (default: per D-05).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
