...
def _make_synthetic_pipeline(
    tmp_path: Path, X: np.ndarray, y: np.ndarray, name: str,
) -> Path:
...
def test_gate_verify_happy_path_writes_verdict_json(
    ...
    result = runner.invoke(
        ...
        [
            "gate", "verify",
            "--candidate", str(p_cand),
            "--canonical", str(p_canon),
            "--substrate-parquet", str(p_sub),
        ],
    )
...
def test_gate_verify_malformed_substrate_exits_1_with_clean_message(
    ...
    invalid_table = pa.table(
        {
            ...
            "substrate_sha": pa.array(
                ["sha-s1", "sha-s1", "sha-s1"], type=pa.string(),
            ),
        }
    )
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--candidate", str(p_cand),
            "--canonical", str(p_canon),
            "--substrate-parquet", str(invalid_substrate),
        ],
    )
...
def test_gate_verify_missing_substrate_path_exits_cleanly(
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--candidate", str(p_cand),
            "--canonical", str(p_canon),
            "--substrate-parquet", str(tmp_path / "nonexistent.parquet"),
        ],
    )
    ...
    assert (
        "nonexistent.parquet" in result.stdout
        or "load failed" in result.stdout
