"""Phase 65 Plan 65-04 (FEAT-V261-02) — end-to-end CLI integration test.

End-to-end integration test for ``ufc gate verify`` against the REAL
Plan 65-03 candidate (``models/meta/meta_v2_refv2.joblib``, 13-wide) and
the REAL canonical (``models/meta/meta_v2.joblib``, 13-wide) using a
freshly-built REF substrate parquet from Plan 65-04's
``build_substrate_parquet`` materialized into ``tmp_path``.

This is the integration validator that proves the four Phase 65 components
interlock correctly with the Phase 63 CLI + Phase 55 verifier:

  - Plan 65-01: ``derive_event_country_bucket`` + ``derive_scoring_regime``
    + ``compute_ref_v2_features_shrunk`` REF v2 feature helpers
  - Plan 65-02: ``scripts/retrain_xgb_v2_refv2.py`` produced
    ``data/intermediate/xgb_v2_refv2_oof.parquet`` (substrate col[0] source)
  - Plan 65-03: ``scripts/train_meta_v2_refv2.py`` produced
    ``models/meta/meta_v2_refv2.joblib`` (the candidate the verifier scores)
  - Plan 65-04 (this plan): ``scripts/build_ref_substrate_v261.py`` produces
    a Phase 63 D-01-compliant 13-wide REF substrate parquet that round-trips
...
Per Plan 65-04 acceptance: this file pins only the SHAPE of the verdict
JSON, NOT the verdict literal. Plan 65-05 owns the verdict-literal
...
``xgb_v2_refv2_oof`` (a different distribution from the canonical
...
set matches Plan 65-04's locked slice names.
...
WR-02 inheritance (Phase 64 Plan 64-03 review-fix): the three tests share
a single module-scope ``verdict_dict`` fixture so the heavy build + verifier
cascade runs exactly once per module run, not once per test.

Skip guard: if either real artifact is missing (sparse-checkout / CI without
LFS, or Plan 65-03 not yet shipped), the whole module is skipped — both
...
# Plan 65-04's builder lives under scripts/ and is not a package; add to
# sys.path so the direct import works. Mirrors Plan 64-03's pattern.
...
# Plan 65-03 candidate — the REF v2 META retrain (13-wide, candidate-aligned).
...
# Phase 65 Plan 65-04 locked slice names (= Phase 64 Plan 64-02 locked
# slice names; both substrates use the same 3-slice convention so per-slice
# metrics line up across the v2.6.1 verifier audit trail).
...
# can emit. Plan 65-05 adjudicates which one is the actual outcome; this
...
# CI / sparse-checkout envs (or if Plan 65-03 has not yet shipped). In dev
# with both artifacts present this skip never triggers.
pytestmark = pytest.mark.skipif(
    not (CANDIDATE.exists() and CANONICAL.exists()),
    reason=(
        f"Real REF artifacts not present "
...
def built_substrate(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """Materialize a fresh REF substrate parquet under a per-module tmp dir.

    Uses Plan 65-04's ``build_substrate_parquet`` with the default
    ``synthetic`` source (DB-free; byte-stable across re-runs via fixed
    RNG seed + REF_SUBSTRATE_REFERENCE_DATE). The returned path is consumed
    by the verifier in the ``verdict_dict`` fixture downstream.

    WR-02 fix (Phase 64 review inheritance): module-scope so the heavy
    ``build_substrate_parquet`` call runs exactly once per test module
    ...
    by-construction (fixed seed + REF_SUBSTRATE_REFERENCE_DATE), so sharing
    the build across tests is safe — no test mutates the parquet.
    """
    tmp_dir = tmp_path_factory.mktemp("ref_substrate_built")
    out = tmp_dir / "ref_substrate.parquet"
...
def verdict_dict(
    ...
    WR-02 fix (Phase 64 review inheritance): module-scope so all three tests
    in this module share the same (substrate-build + verifier-run) cycle —
    exactly once per module run, not once per test. Uses ``tmp_path_factory``
    ...
    """
    tmp_dir = tmp_path_factory.mktemp("ref_verdict_dict")
...
def test_gate_verify_ref_e2e_emits_valid_verdict_shape(
    verdict_dict: dict,
) -> None:
    ...
    keys on a real verifier run — Plan 65-04 substrate vs Plan 65-03
    candidate):
      ...
      - ``candidate_sha``: 64-char hex string (Plan 65-03 anchor)
    ...
    NB: Plan 65-04 pins SHAPE only. Plan 65-05 owns the verdict-literal
    adjudication. High-prior expectation is ``confound_block`` (substrate
    col[0] is xgb_v2_refv2_oof, distribution-shifted vs canonical xgb_v2
...
def test_gate_verify_ref_e2e_canonical_sha_matches_audit01(
    verdict_dict: dict,
) -> None:
...
def test_gate_verify_ref_e2e_slice_set_matches_substrate(
    verdict_dict: dict,
) -> None:
    """The verifier's per-slice metric tables cover all three Plan 65-04 slices.
    ...
    for field in (
        ...
        assert slice_keys == EXPECTED_SLICE_NAMES, (
            ...
            f"expected {EXPECTED_SLICE_NAMES}. The Plan 65-04 substrate "
