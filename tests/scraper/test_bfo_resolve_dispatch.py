...
def test_resolve_fight_i_picks_2003_event_date():
    ...
    resolved = ingester._resolve_fight(
        COUTURE_DB_ID, LIDDELL_DB_ID, rows[0].event_date
    )
...
def test_resolve_fight_ii_picks_2005_event_date():
    ...
    assert resolved == COUTURE_LIDDELL_II_FIGHT_ID, (
        "Closest-date matching must pick II"
    )
...
def test_resolve_fight_iii_picks_2006_event_date():
    ...
    assert resolved == COUTURE_LIDDELL_III_FIGHT_ID, (
        "Closest-date matching must pick III"
    )
...
def test_resolve_fight_outside_window_returns_none():
    ...
    resolved = ingester._resolve_fight(
        COUTURE_DB_ID, LIDDELL_DB_ID, date(2010, 1, 1)
    )
...
def test_resolve_fight_no_candidates_returns_none():
    ...
    assert ingester._resolve_fight(
        COUTURE_DB_ID, LIDDELL_DB_ID, COUTURE_LIDDELL_I_EVENT_DATE
    ) is None
